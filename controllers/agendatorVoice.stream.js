...
exports.handleVoiceStream = [
    ...
    async (req, res) => {
        try {
            ...
            safeUnlink(realPath);
            logger.info('[agVoice] texto: ' + text.slice(0, 80) + '…');

            const { model = 'gpt-4.1-mini', noSearch, id_paciente } = req.body || {};
            ...
            if (req.session.agSelectedPatient)
            ...
            if (fullText && fullText !== '[Sin respuesta]') {
                ...
                } catch (e) { logger.error('[agVoice TTS] ' + e.message); }
            }
            ...
            logger.error('[agVoiceStream] ' + err.stack);
