...
async function loadSessionInfo(req) {
  ...
  const [pRows] = await pool.query(
    `SELECT id_paciente, fecha_nacimiento, fecha_registro,
     ...
     FROM pacientes
     WHERE id_paciente = ? LIMIT 1`, [patient.id_paciente]);
  ...
  const [evRows] = await pool.query(
    `SELECT e.*, pr.nombre AS prof_nombre
       ...
       WHERE e.id_paciente = ?
       ORDER BY e.fecha_evaluacion DESC, e.id_evaluacion DESC LIMIT 1`,
       [patient.id_paciente]);
  ...
  const [trRows] = await pool.query(
    `SELECT t.*, pr.nombre AS prof_nombre
       ...
       WHERE t.id_paciente = ?
       ORDER BY t.fecha_inicio DESC, t.id_tratamiento DESC LIMIT 1`,
       [patient.id_paciente]);
  ...
  const [seRows] = await pool.query(
    `SELECT s.*, pr.nombre AS prof_nombre
       ...
       WHERE t.id_paciente = ?
       ORDER BY s.fecha_sesion DESC, s.hora_sesion DESC, s.id_sesion DESC LIMIT 1`,
       [patient.id_paciente]);
...
const LOCAL_FUNCTIONS = {
 ...
 /* ────────── PDF CORPORATIVO GENÉRICO ────────── */
  ...
  /* ────────── CONSENTIMIENTO | LOPD ────────── */
  /**
   * Genera el documento de consentimiento LOPD
   * y devuelve { message, pdfURL, setCode } cuando está listo.
   *
   * Args opcionales:
   *   - nombreArchivo   (string .pdf)
  ...
  async create_consentimiento_lopd (args = {}, req) {
    if (!req.session.patient)
      throw new Error('Sesión sin paciente');

    const p = req.session.patient;            // nombre, apellidos, dni, email vienen de aquí
    const payload = {
      nombrePaciente : `${p.nombre} ${p.apellidos}`.trim(),
      dniPaciente    : p.dni || '—',
      emailPaciente  : p.email || 'info@parisandbea.es',
      ...args        // nombreArchivo, fechaDocumento…
    };

    /* promesa que aguarda a que mockRes.json sea llamado */
    return await new Promise((resolve, reject) => {
      const mockReq = { body: payload };
      const mockRes = {
        statusCode: 200,
        ...
        json   (obj){
          if (this.statusCode >= 200 && this.statusCode < 300)
            return resolve(obj);
          return reject(new Error(obj?.error || 'Error generando consentimiento LOPD'));
        }
      };
  ...
  /* ────────── BUSCAR PACIENTES ────────── */
  /**
   * Devuelve una lista de pacientes que cumplen los filtros:
   *   - nombre (string, parcial)
   *   - apellidos (string, parcial)
   *   - telefono (string, parcial)
   *   - dni (string, parcial)
   *   - startDate / endDate (yyyy-mm-dd)
   */
  async search_pacientes (filtros = {}, _req) {
    /* Reutilizamos el controlador tal cual */
    return await new Promise((resolve, reject) => {
      const mockReq = { query: filtros };
      const mockRes = {
        statusCode: 200,
        status (c){ this.statusCode = c; return this; },
        json   (obj){
          if (this.statusCode >= 200 && this.statusCode < 300) return resolve(obj);
          reject(new Error(obj?.error || 'Error buscando pacientes'));
        }
      };
      searchPatients(mockReq, mockRes).catch(reject);
    });
  },

  /* ────────── ACTUALIZAR PACIENTE ────────── */
  /**
   * Actualiza datos personales de un paciente.
   *   Args:
   *     id_paciente (int, obligatorio)
   *     campos a modificar (cualquiera de: nombre, apellidos, fecha_nacimiento,
   *                        genero, dni, direccion, telefono, email)
   * Devuelve { message }
   */
  async update_paciente (args = {}, _req) {
    const { id_paciente, ...campos } = args;
    if (!id_paciente) throw new Error('Falta id_paciente');

    return await new Promise((resolve, reject) => {
      const mockReq = { params:{ id:id_paciente }, body:campos };
      const mockRes = {
        statusCode: 200,
        status (c){ this.statusCode = c; return this; },
        json   (obj){
          if (this.statusCode >= 200 && this.statusCode < 300) return resolve(obj);
          reject(new Error(obj?.error || 'Error actualizando paciente'));
        }
      };
      updatePatient(mockReq, mockRes).catch(reject);
    });
  },
	/* ────────── ENVIAR EMAIL ────────── */
